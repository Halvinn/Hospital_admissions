...
The dataset has `r nrow(discharges)` records and `r length(names(discharges))` columns.
...
```{r}
options(scipen=999)
sort(sapply(discharges, function(x) sum(is.na(x)) / length(x) * 100), decreasing = TRUE)
```
...
library(Amelia)
missmap(discharges, y.labels = NULL, y.at = NULL, x.cex = 0.8)
```

```{r}
filter(discharges, is.na(`Facility Name`))
```

```{r}
count(filter(discharges, `Abortion Edit Indicator` == "Y"))
```
...
```{r}
drop_cols = c("Other_Provider_License_Number", "Zip_Code_-_3_digits")
discharges_clean <- discharges %>%
    select(-one_of(drop_cols)) %>% # Columns dropped.
    filter(!is.na(Facility_Id)) # Row with missing info excluded.
```

Explore missing data in `Payment_Typology_1` and `Payment_Typology_2` columns.

```{r}
discharges_clean %>% filter(is.na(Payment_Typology_3)) %>%
    select(c(Facility_Id, Facility_Name, Total_Charges, Payment_Typology_1,
...
```{r}
dim(discharges_clean)
```

# Univariate Analysis
Analysis of indiviadual variables wil give us a better understanding of the data by
exploring such things as distribution or summaries of variables. Boxplots and density 
plots are a good way of visually communicating distributiong of data.


```{r}
ggplot(discharges_clean, aes(x = 1, y = Length_of_Stay)) +
    geom_boxplot()
```
